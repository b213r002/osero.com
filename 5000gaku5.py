...
# コンパイル
model.compile(optimizer=optimizers.Adam(learning_rate=0.0008),  # SGDからAdamに変更
              loss='categorical_crossentropy', 
              metrics=['accuracy'])


# Tensor Boardコールバックの設定
tb_cb = keras.callbacks.TensorBoard(log_dir='model_log/relu_1008.5', histogram_freq=1, write_graph=True)

start_time = time.time()  # 訓練開始時間を記録

model.fit(x_train, y_train_reshape, epochs=200, batch_size=32, validation_split=0.2, callbacks=[tb_cb, early_stopping], verbose=1)
model.save('saved_model/my_model5000@block8.103.new')
