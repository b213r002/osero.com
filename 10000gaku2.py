...
# CSVファイルの読み込み (ヘッダーなし)
csv_data = pd.read_csv("10000.csv", header=None)
...
model.add(layers.Conv2D(64, kernel_size=3, padding='same', activation='relu'))  # Conv2D層1
...
early_stopping = keras.callbacks.EarlyStopping(
    ...
    patience=7,          # 指標が改善しないエポック数
    restore_best_weights=True  # 最良の重みを復元
)


# コンパイル
model.compile(optimizer=optimizers.Adam(learning_rate=0.0007),  # SGDからAdamに変更
...
# Tensor Boardコールバックの設定
tb_cb = keras.callbacks.TensorBoard(log_dir='model_log/relu_10000.5', histogram_freq=1, write_graph=True)

start_time = time.time()  # 訓練開始時間を記録

model.fit(x_train, y_train_reshape, epochs=300, batch_size=32, validation_split=0.25, callbacks=[tb_cb, early_stopping], verbose=1)
model.save('saved_model/my_model10000@block7.103.5')
